...
const isConnected = function isConnected(element: Element) {
...
const isEdge = isBrowser && /Edge|EdgA/.test(navigator.userAgent);

export default function getCaretCoordinates(
    element: any,
    position: number,
    options: any): Coordinates {
    if (!element) {
        ...
        if (special) {
            ...
            if (window.getSelection()?.rangeCount) {
                ...
                if (modified) {
                    ...
                    if (document.activeElement!.firstChild != null)
